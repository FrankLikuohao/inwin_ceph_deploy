...
if (( $# < 3 )); then
    ...
    echo "ex: $0 hostname 2 /dev/sdd -r"
...
SDX_ARR=($(\ssh $host "cat  notssdlist.txt "))
for ((i=0; i<$skipsd; i++)); do unset SDX_ARR[$i]; done
echo ${SDX_ARR[@]}

SSD_ARR=($(\ssh $host "ls -m $ssd_location* | tr ',' ' '| tr ':' ' '"))
unset SSD_ARR[0]
echo ${SSD_ARR[@]}
#unset arr[${#arr[@]}-1]
...
for ((i=0; i<$howmanyosd; i++)); do 
	osdn=$i+$skipsd
	journaln=$i+1
	echo "ceph-deploy disk zap $host:${SDX_ARR[$osdn]}">> $execfile
	echo "ceph-deploy osd prepare $host:${SDX_ARR[$osdn]}:${SSD_ARR[$journaln]}" >> $execfile
	# ceph-deploy osd activate vmon1:/dev/sdc1:/dev/sdd1
	echo "ceph-deploy osd activate $host:${SDX_ARR[$osdn]}1:${SSD_ARR[$journaln]}" >>$execfile
done
chmod a+x $execfile
more $execfile
exit 0;
